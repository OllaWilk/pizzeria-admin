...
//dataSource. To definicje, właściwośći/opisy/ naszych produktów.
//Te dane będą wczytywane przez naszą stronę z tego pliku, ale chcę by w przyszłości
//były wczytywane z serwera. przygotowanie pod to mam w script. js metoda app.initData
//dane z data.js są przygotowane pod index.html
dataSource.products = {
  cake: {
    class: 'small',
    name: 'Zio Stefano\'s Doughnut',
    price: 9,   //cena produktu uwzględnia domyślne opcje. Przy zmianie opcji nie można doliczać tej ceny jak i przy odnaczeniu domyślnych opcji cena produktu powinna się zmienić.
    ...
    ], //ciasto nie ma właściwości ponieważ jest tylko jeden rodzaj ciast
  },
  breakfast: {
    class: 'small',
    ...
    params: {     //właściwości śniadania kawa=(etykieta rodzaj, opcje(Late(etykieta, cena ,chcemleko:true v niechcemleka:falce), cappucino(etykieta,cena),esspresso(etykieta,cena))
      coffee: {
        label: 'Coffee type', //nazwa wyświetlana na stronie
        type: 'radios',   //typ parametru czy wyświetli się lista checkbox
        options: {      //zestawy możliwych opcji do wybrania
          latte: {label: 'Latte', price: 1, default: true},  // default, czyli opcjonalne ustawienie, które mówi, że ta opcja ma być domyślnie wybrana i została już wliczona w cenę tego produktu.
  ...
  pizza: {
    ...
    images: [
      '<img class="active" src="images/pizza/crust.svg">', //NIE USUWAJ KLASY ACTIVE. CIASTO MA BYĆ ZAWSZE WIDOCZNE
      '<img class="active sauce-tomato" src="images/pizza/tomato-sauce.svg">', // druga klasa ta 2członowa to jest kluch parametru - klucz opcji
    ...
    params: {   //w htmlu w checkbox w atrybucie name="{{ paramId }}" mamy klucze parametrów pizzy
      sauce: {
        label: 'Sauce',
        type: 'radios',
        options: {
          tomato: {label: 'Tomato', price: 0, default: true}, //w htmlu w checkbox w atrybucie value="{{ @key }}" mamy klucze danej opcji(wybory np.  w sosie będzie to rodzaj sosu np tomato)
